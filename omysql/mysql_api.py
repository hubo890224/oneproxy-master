...
from omysql.models import MysqlServer,MysqlGroup,MYSQL_STATUS,MYSQL_ENV
...
def group_add_mysql(group, mysql_id=None, mysql_ip=None):
    """
    资产组添加资产
    ...
    if mysql_id:
        ...
        mysql = get_object(MysqlServer, mysql_ip=mysql_ip)

    if mysql:
        group.mysqlserver_set.add(mysql)
...
def db_add_group(**kwargs):
    ...
    group = get_object(MysqlGroup, name=name)
    mysql_id_list = kwargs.pop('mysql_select')

    if not group:
        group = MysqlGroup(**kwargs)
        group.save()
        for mysql_id in mysql_id_list:
            group_add_mysql(group, mysql_id)
...
def db_update_group(**kwargs):
    ...
    MysqlGroup.objects.filter(id=group_id).update(**kwargs)
...
def db_mysql_alert(mysql, username, alert_dic):
    """
    asset alert info to db
    """
    mysql_list = []
    mysql_tuple_dic = {'status': MYSQL_STATUS, 'env': MYSQL_ENV}
    for field, value in alert_dic.iteritems():
        field_name = MysqlServer._meta.get_field_by_name(field)[0].verbose_name
        if field == 'idc':
        ...
        elif field in ['status', 'env']:
            alert_info = get_tuple_diff(mysql_tuple_dic.get(field), field_name, value)

        elif field == 'group':
            old, new = [], []
            for group_id in value[0]:
                group_name = MysqlServer.objects.get(id=int(group_id)).name
                old.append(group_name)
            for group_id in value[1]:
                group_name = MysqlGroup.objects.get(id=int(group_id)).name
        ...
        if 'alert_info' in dir():
            mysql_list.append(alert_info)

    #if mysql_list:
    #    AssetRecord.objects.create(asset=asset, username=username, content=mysql_list)
...
def ansible_record(asset, ansible_dic, username):
    ...
    db_mysql_alert(asset, username, alert_dic)
...
def connect_mysql(hostname,username,password,port):
    try:
        conn = MySQLdb.connect(host=hostname, user=username, passwd=password, port=int(port), charset='utf8')
        return  conn
...
def get_variables(hostname,username,password,port):
    conn = connect_mysql(hostname,username,password,port)

    cur=conn.cursor()

    cur.execute('show variables where Variable_name in ("innodb_flush_log_at_trx_commit", "innodb_flush_method", "innodb_buffer_pool_size","innodb_max_dirty_pages_pct", "innodb_log_buffer_size", "innodb_log_file_size","innodb_log_files_in_group", "innodb_thread_concurrency", "innodb_file_per_table","innodb_adaptive_hash_index", "innodb_open_files", "innodb_io_capacity","innodb_read_io_threads", "innodb_write_io_threads", "innodb_adaptive_flushing","innodb_lock_wait_timeout", "innodb_log_files_in_group")')

    mysql_options = cur.fetchall()

    return mysql_options

def get_performance_info(hostname,username,password,port):
    performance_sql="show global status where Variable_name in ('Com_select','Com_insert','Com_update','Com_delete')"
    conn = connect_mysql(hostname, username, password, port)
    cur = conn.cursor()
    performance_info = cur.execute(performance_sql)
